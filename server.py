        ...
        if self.login is not None:
            ...
            if message.startswith("login:"):
                ...
                if self.login in self.names:
                    print(self.names)  # показывает список
                    self.transport.write("But this login is buzy already. Please reconnect to server with correct login :(\n".encode())
                    self.factory.clients.remove(self)


                notification = f"New user connected: {self.login}"
                self.names.append(self.login)  # добавляет в список пользователей
